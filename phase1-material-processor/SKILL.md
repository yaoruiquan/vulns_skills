...
| 2 | 重命名文件夹 | 改为"杭州安恒信息原创漏洞报送N个-时间戳" |
...
将文件夹名改为 `杭州安恒信息原创漏洞报送N个-时间戳`（时间戳使用原始文件夹名，避免重名冲突）：

```bash
mv "/path/to/原始文件夹名" "/path/to/杭州安恒信息原创漏洞报送N个-原始文件夹名"
```

示例：原始文件夹 `2026-04-13-162037`，7个漏洞 → `杭州安恒信息原创漏洞报送7个-2026-04-13-162037`
...
| CNVD | 漏洞分析有开头段落和官网链接后缀，原内容和表格保留 |
| CNVD | 漏洞验证过程已替换为标准语句 |
