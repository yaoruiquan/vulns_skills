...
def map_cnvd_vuln_type(vuln_type_text: str) -> str:
    ...
    normalized_text = text.replace(" ", "").replace("-", "").replace("_", "").lower()
    for option in CNVD_VULN_TYPE_OPTIONS:
        if option.lower() in normalized_text:
            return option
    ...
    for key, val in mapping:
        if key.replace(" ", "").replace("-", "").replace("_", "").lower() in normalized_text:
            return val
    return "其他"
