...
def title_from_dir_name(name: str) -> str:
    """从 DAS/CNVD 目录名推断漏洞名称。"""
    cleaned = re.sub(r"^DAS-[A-Z]?\d+-", "", name or "")
    cleaned = re.sub(r"^CNVD-", "", cleaned)
    cleaned = re.sub(r"^CNVD-(?:C-)?\d{4}-\d+-DAS-[A-Z]?\d+-?", "", cleaned)
    return cleaned.strip("-") or name


def find_nearby_context(zip_path: Path) -> dict:
    """直接传 zip 时，尝试读取同一 DAS 目录下 CNVD 子目录的 form_context.json。"""
    ...
    for root in roots:
        ...
        if direct.is_file():
            try:
                ...
                if context.get("platform") == PLATFORM or "CNVD-" in direct.parent.name:
                    return context
            except (OSError, json.JSONDecodeError):
                pass

        for child in root.iterdir():
            if not child.is_dir() or not child.name.startswith("CNVD-"):
                continue
...
def resolve_zip_and_context(target: str, vuln_name: str = "") -> tuple[Path, dict]:
    ...
    raise SystemExit("输入必须是 form_context.json 或 CNVD-*.zip")


def validate_zip(zip_path: Path, *, allow_non_prefixed: bool = False) -> None:
    """校验上传文件必须是单个 CNVD zip。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="上传单个 CNVD 原始 zip，并可推送钉钉下载链接")
    parser.add_argument("target", help="CNVD form_context.json 或 CNVD-*.zip 路径")
    parser.add_argument("--platform-id", default="", help="提交成功后的 CNVD 编号")
    ...
    parser.add_argument("--allow-non-prefixed", action="store_true", help="允许上传文件名不是 CNVD 开头的 zip")
    ...
    parser.add_argument("--title", default="监管上报 CNVD 附件已上传", help="钉钉消息标题")
