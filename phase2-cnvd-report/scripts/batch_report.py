...
def build_notify_text(state: dict, results: list[dict]) -> tuple[str, list[str]]:
    ...
    for index, result in enumerate(results, start=1):
        platform_id = result.get("platform_id") or "未记录"
        lines.extend([
            f"{index}. {result.get('das_id', '')} / {platform_id}",
            f"漏洞名称：{result.get('vuln_name', '')}",
            f"附件：{result.get('zip_name', '')}（{result.get('zip_size_mb', 0)} MB）",
        ])
