...
### 3.1 先只处理两个下拉框：漏洞所属类型 + 漏洞类型

**关键步骤**：先只切换“漏洞所属类型”，再只选择“漏洞类型”。在这两个下拉框选完、页面联动刷新完成之前，不要提前填写其他文本字段。

```
MCP: take_snapshot

# 第一步：先切换到 form_context.json 的漏洞所属类型
MCP: fill
  uid: "<漏洞所属类型下拉框的 uid>"
...
完成这一步后，再 `take_snapshot` 一次确认页面已刷新到目标表单状态，然后直接按 `page_payloads` 统一填写其余字段。除这次联动确认外，不要为单个字段反复 `take_snapshot`。
...
2. 如验证码刷新较快，提前启动常驻 OCR 服务；优先使用 `form_context.json.ocr.start_command`：

```bash
python3 scripts/captcha_ocr.py --serve --port 18765
```

3. 先主动点击一次验证码图片刷新，确保拿到最新验证码：

```text
MCP: evaluate_script
  function: |
    () => {
      const image = document.querySelector("#codeSpan1 img");
      if (!image) return { ok: false, reason: "未找到验证码图片 #codeSpan1 img" };
      image.click();
      return { ok: true, src: image.src };
    }
```
...
5. 识别结果返回后，不要再执行 `take_snapshot` 或长时间等待；用同一次 `evaluate_script` 完成填入验证码和点击提交。
...
6. 如果 OCR 识别失败，或页面提示验证码错误，不要直接复用当前小图。先把验证码图片在新标签页打开，再对新标签页中的大图截图识别：
...
这样可以避免“识别还没填入，验证码已经刷新”的问题。验证码错误时，重新刷新当前验证码，必要时在新标签页打开大图后再重试，不复用旧结果。
