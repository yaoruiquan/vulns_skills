...
当前目录自带 `./.mcp.json`，指向当前 skill 的本地 wrapper：
...
| Skill | 端口 | Profile 名 | 建议同项目 MCP 名 |
|-------|------|-----------|------------------|
| 漏洞预警 | 9331 | vuln-alert | vuln-alert-chrome |
| CNVD | 9332 | cnvd-report | cnvd-chrome |
| CNNVD | 9333 | cnnvd-report | cnnvd-chrome |

修改方式：在 `.env` 中设置 `CLAUDE_CHROME_MCP_PORT` 和 `CLAUDE_CHROME_PROFILE_NAME`。不要直接改 wrapper，除非是在复制出新 skill 后做结构性定制。
...
3. 创建 `.env` 配置数据目录
4. 如需不同端口，修改端口配置
5. 如果要在同一 Claude 项目里并发加载，使用唯一 MCP server 名注册
6. 启动 Chrome 并测试
