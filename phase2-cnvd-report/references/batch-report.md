...
按 DAS 目录名称排序上报。只处理 `CNVD-*` 子目录，忽略 `CNNVD-*` 子目录。
...
```text
/tmp/vulns-skills/phase2-cnvd-report/batches/<批次名>/batch_state.json
```
...
按输出的 `prepare_context_command` 生成本条 `form_context.json`，然后执行单个 CNVD 上报流程。环境检查完成后标记一次：
