...
5. 标题、摘要、作者、封面属于草稿元数据；正文首屏只允许先放固定横幅 `assets/logo.png`，随后进入漏洞概述表格，不插入源 Markdown 的 logo、封面图或独立大标题。
...
正文开头先渲染固定横幅 `assets/logo.png`，再直接渲染“漏洞概述”表格。不要把源 Markdown 的 `logo-*.JPG`、复现截图、公众号封面图、文章标题或 `<!-- IMG:banner -->` 放入正文首屏；封面只用于公众号草稿元数据。
