...
9. 草稿创建完成后，返回标题、HTML 路径、封面路径和草稿创建结果。
...
- 封面标题必须使用“漏洞名称 + CVE”格式，不带 `【已复现】`、`【风险通告】` 等正文前缀；例如 `Linux Kernel Fragnesia 本地权限提升漏洞（CVE-2026-46300）`。
...
- 公众号后台会在正文上方显示草稿标题；HTML 正文开头只使用 skill 固定横幅 `assets/logo.png`，随后进入“漏洞概述”表格。不要在正文内再插入独立大标题，也不要插入源 Markdown 的 logo 或封面图。
