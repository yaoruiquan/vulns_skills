...
- `assets/wechat-alert-article-template.placeholders.html`：确定性 HTML 占位符模板。生成正文时由脚本填充。
...
2. 使用脚本生成 HTML，不要让 agent 手写正文 HTML：

   ```bash
   python3 scripts/render_wechat_article.py article.md --output /tmp/article.html --json
   ```

3. 脚本会解析标题、漏洞概述表、修复方案、参考资料、产品能力覆盖，并填充 `assets/wechat-alert-article-template.placeholders.html`。同时会在 HTML 旁生成 `<html>.meta.json`，用于草稿标题、作者和摘要元数据。
...
8. 只有 `WECHAT_APPID`、`WECHAT_SECRET` 和封面都可用时，才创建公众号草稿。由于 HTML 由 agent 直接生成，优先使用 `./scripts/md2wechat-env.sh test-draft <html> <cover>`；当 `<html>.meta.json` 存在时，wrapper 会自动构造带真实标题的 `create_draft <json>` 请求。不要默认走 `convert --draft`。
